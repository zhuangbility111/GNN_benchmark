...
def divide_remote_edges_list(begin_node_on_each_subgraph, remote_edges_list, world_size):
    ...
    for i in range(world_size):
        ...
        if num_src_nodes > num_dst_nodes:
            ...
            remote_edges_list_post_aggr_from[0].append(src_in_remote_edges)
            remote_edges_list_post_aggr_from[1].append(dst_in_remote_edges)
...
def transform_edge_index_to_sparse_tensor(local_edges_list, \
    ...
    print("start to remap nodes id for pre_aggr_to")
...
def test_forward(test_model, graph, feats):
    ...
    for epoch in range(1):
        ...
        print(out)
        nodes_label_list = torch.tensor([2, 3, 0], dtype=torch.int64)
...
if __name__ == "__main__":
    ...
    local_nodes_list, nodes_feat_list, \
    ...
    hidden_channels = 8
    output_channels = 4
    max_size = max(input_channels, hidden_channels)
