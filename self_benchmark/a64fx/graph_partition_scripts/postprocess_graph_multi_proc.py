...
def split_nodes_feats(in_dir, out_dir, graph_name, begin_part, end_part):
    # node_range_on_each_part = []
    # node_range_on_each_part.append(0)
    for i in range(begin_part, end_part):
        # node_id_list = np.loadtxt(os.path.join(out_dir, "p{:0>3d}-{}_nodes.txt".format(i, graph_name)), dtype='int64', delimiter = ' ', usecols=(0, 3))
        node_id_list = pd.read_csv(os.path.join(out_dir, "p{:0>3d}-{}_nodes.txt".format(i, graph_name)), sep=" ", header=None, usecols=[0, 4], dtype='int64').values
        # save the node_id_list to npy file
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_nodes.npy".format(i, graph_name)), node_id_list)
        begin_idx = node_id_list[0][0]
        num_nodes = node_id_list.shape[0]
        end_idx = node_id_list[num_nodes-1][0]

        node_feat_list = np.load(os.path.join(in_dir, "{}_nodes_feat.npy".format(graph_name)), mmap_mode='r')
        local_node_feat_list = node_feat_list[node_id_list[:, 1]]
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_nodes_feat.npy".format(i, graph_name)), local_node_feat_list)
        print(local_node_feat_list.shape)

        node_label_list = np.load(os.path.join(in_dir, "{}_nodes_label.npy".format(graph_name)), mmap_mode='r')
        local_node_label_list = node_label_list[node_id_list[:, 1]].reshape(-1).astype(np.int64)
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_nodes_label.npy".format(i, graph_name)), local_node_label_list)
        print(local_node_label_list.shape)

        edges_list = pd.read_csv(os.path.join(out_dir, "p{:0>3d}-{}_edges.txt".format(i, graph_name)), sep=" ", header=None, usecols=[0, 1], dtype='int64').values
        ...
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_local_edges.npy".format(i, graph_name)), local_edges_list)
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_remote_edges.npy".format(i, graph_name)), remote_edges_list)
...
def split_node_datamask(in_dir, out_dir, graph_name, begin_part, end_part):
    remap_start = time.perf_counter()
    train_idx = np.load(os.path.join(in_dir, "{}_nodes_train_idx.npy".format(graph_name)))
    valid_idx = np.load(os.path.join(in_dir, "{}_nodes_valid_idx.npy".format(graph_name)))
    test_idx = np.load(os.path.join(in_dir, "{}_nodes_test_idx.npy".format(graph_name)))
    for i in range(begin_part, end_part):
        # nodes_id_list = pd.read_csv(os.path.join("./", "p{:0>3d}-{}_nodes.txt".format(i, graph_name)), sep=" ", header=None, usecols=[0, 3]).values
        nodes_id_list = np.load(os.path.join(out_dir, "p{:0>3d}-{}_nodes.npy".format(i, graph_name)))
        ...
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_nodes_train_idx.npy".format(i, graph_name)), local_train_idx)
        # np.savetxt(os.path.join(dir_path, "p{:0>3d}-{}_nodes_valid_idx.txt".format(i, graph_name)), local_valid_idx, fmt = "%d", delimiter = ' ')
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_nodes_valid_idx.npy".format(i, graph_name)), local_valid_idx)
        # np.savetxt(os.path.join(dir_path, "p{:0>3d}-{}_nodes_test_idx.txt".format(i, graph_name)), local_test_idx, fmt = "%d", delimiter = ' ')
        np.save(os.path.join(out_dir, "p{:0>3d}-{}_nodes_test_idx.npy".format(i, graph_name)), local_test_idx)

    remap_end = time.perf_counter()
    print("elapsed time of ramapping dataset mask(ms) = {}".format((remap_end - remap_start) * 1000))

def combined_func(in_dir, out_dir, graph_name, begin_part, end_part):
    split_nodes_feats(in_dir, out_dir, graph_name, begin_part, end_part)
    split_node_datamask(in_dir, out_dir, graph_name, begin_part, end_part)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-o', '--out_dir', type=str, default='./', help='The dir of output.')
    parser.add_argument('-i', '--in_dir', type=str, default='./', help='The path of input original data.')
    ...
    for pid in range(num_process):
        p = Process(target=combined_func,
                    args=(in_dir, out_dir, graph_name, begin_part + pid*step, min((begin_part + (pid+1)*step), end_part)))
        p.start()
    ...
    np.savetxt(os.path.join(out_dir, "begin_node_on_each_partition.txt"), node_range_on_each_part.reshape(1, -1), fmt = "%d", delimiter = ' ')
