...
class DataProcessor(object):
    ...
    def obtain_local_nodes_required_by_other(
        ...
        recv_nodes_list = [
            torch.zeros(num_local_nodes_required_by_other[i], dtype=torch.int64) for i in range(world_size)
        ]
        if world_size != 1:
...
class DataProcessorForPre(object):
    ...
    def process_remote_edges_pre_post_aggr_to(is_pre_post_aggr_to, remote_edges_pre_post_aggr_to, world_size):
        ...
        for part_id in range(world_size):
            ...
            if is_pre_post_aggr_to[part_id][0].item() == 0:
                ...
                remote_edges_list_pre_post_aggr_to[0] = torch.cat(
                    (remote_edges_list_pre_post_aggr_to[0], remote_edges_pre_post_aggr_to[part_id]), dim=0
                )
                remote_edges_list_pre_post_aggr_to[1] = torch.cat(
                    (remote_edges_list_pre_post_aggr_to[1], remote_edges_pre_post_aggr_to[part_id]), dim=0
                )
                # collect it for remapping nodes id
                begin_edge_on_each_partition_to[part_id + 1] = (
                    begin_edge_on_each_partition_to[part_id] + remote_edges_pre_post_aggr_to[part_id].shape[0]
                )
            # pre_aggregate
            else:
                ...
                remote_edges_list_pre_post_aggr_to[0] = torch.cat(
                    (remote_edges_list_pre_post_aggr_to[0], src_in_remote_edges[sort_index]), dim=0
                )
                remote_edges_list_pre_post_aggr_to[1] = torch.cat(
                    (remote_edges_list_pre_post_aggr_to[1], dst_in_remote_edges[sort_index]), dim=0
                )
                # collect it for remapping nodes id
                begin_edge_on_each_partition_to[part_id + 1] = (
                    begin_edge_on_each_partition_to[part_id] + dst_in_remote_edges.shape[0]
                )
    ...
    def divide_remote_edges_list(begin_node_on_each_subgraph, remote_edges_list, world_size):
        ...
        for i in range(world_size):
            ...
            if num_src_nodes > num_dst_nodes:
                ...
                tmp = torch.cat((src_in_remote_edges, dst_in_remote_edges), dim=0)
                ...
                remote_edges_list_pre_post_aggr_from[0] = torch.cat(
                    (remote_edges_list_pre_post_aggr_from[0], ids_dst_nodes), dim=0
                )
                remote_edges_list_pre_post_aggr_from[1] = torch.cat(
                    (remote_edges_list_pre_post_aggr_from[1], ids_dst_nodes), dim=0
                )
                # collect it for remapping nodes id
                begin_edge_on_each_partition_from[i + 1] = (
                    begin_edge_on_each_partition_from[i] + ids_dst_nodes.shape[0]
                )
            else:
                # post_aggr
                ...
                remote_edges_list_pre_post_aggr_from[0] = torch.cat(
                    (remote_edges_list_pre_post_aggr_from[0], src_in_remote_edges[sort_index]), dim=0
                )
                remote_edges_list_pre_post_aggr_from[1] = torch.cat(
                    (remote_edges_list_pre_post_aggr_from[1], dst_in_remote_edges[sort_index]), dim=0
                )
                # collect it for remapping nodes id
                begin_edge_on_each_partition_from[i + 1] = (
                    begin_edge_on_each_partition_from[i] + src_in_remote_edges.shape[0]
                )
        ...
        remote_edges_pre_post_aggr_to = [
            torch.empty((indices[1]), dtype=torch.int64) for indices in is_pre_post_aggr_to
        ]
        ...
        (
            ...
            post_aggr_to_splits,
            pre_aggr_to_splits,
        ) = DataProcessorForPre.process_remote_edges_pre_post_aggr_to(
            is_pre_post_aggr_to, remote_edges_pre_post_aggr_to, world_size
        )
    ...
    def transform_edge_index_to_sparse_tensor(
        ...
        num_remote_nodes_from = DataProcessorForPre.remap_remote_nodes_id(
        ...
        num_remote_nodes_to = DataProcessorForPre.remap_remote_nodes_id(
...
class DataProcessorForPreAggresive(object):
    ...
    def process_remote_edges_pre_post_aggr_to(remote_edges_pre_post_aggr_to, world_size):
        ...
        for i in range(world_size):
            ...
            remote_edges_list_pre_post_aggr_to[0] = torch.cat(
                (remote_edges_list_pre_post_aggr_to[0], src_in_remote_edges), dim=0
            )
            remote_edges_list_pre_post_aggr_to[1] = torch.cat(
                (remote_edges_list_pre_post_aggr_to[1], dst_in_remote_edges), dim=0
            )

            begin_edge_on_each_partition_to[i + 1] = (
                begin_edge_on_each_partition_to[i] + dst_in_remote_edges.shape[0]
            )
        begin_edge_on_each_partition_to[world_size] = remote_edges_list_pre_post_aggr_to[0].shape[0]
    ...
    def divide_remote_edges_list(begin_node_on_each_subgraph, remote_edges_list, world_size):
        ...
        for i in range(world_size):
            ...
            src_to_send = torch.cat((src_in_remote_edges_pre_aggr, src_in_remote_edges_post_aggr), dim=0)
            dst_to_send = torch.cat((dst_in_remote_edges_pre_aggr, src_in_remote_edges_post_aggr), dim=0)
            ...
            src_from_recv = torch.cat((src_in_remote_edges_post_aggr, dst_from_recv), dim=0)

            # combine the dst of remote edges which are pre_aggr and post_aggr
            dst_from_recv = torch.cat((dst_in_remote_edges_post_aggr, dst_from_recv), dim=0)
            ...
            remote_edges_list_pre_post_aggr_from[0] = torch.cat(
                (remote_edges_list_pre_post_aggr_from[0], src_from_recv), dim=0
            )
            remote_edges_list_pre_post_aggr_from[1] = torch.cat(
                (remote_edges_list_pre_post_aggr_from[1], dst_from_recv), dim=0
            )

            begin_edge_on_each_partition_from[i + 1] = (
                begin_edge_on_each_partition_from[i] + src_from_recv.shape[0]
            )
        ...
        if world_size != 1:
            ...
            dist.all_to_all_single(
                num_remote_edges_pre_post_aggr_to,
                num_remote_edges_pre_post_aggr_from,
                recv_splits,
                send_splits,
            )
        ...
        remote_edges_pre_post_aggr_to = torch.empty(
            (num_remote_edges_pre_post_aggr_to.sum().item()), dtype=torch.int64
        )
        ...
        if world_size != 1:
            # dist.all_to_all(remote_edges_pre_post_aggr_to, remote_edges_pre_post_aggr_from)
            dist.all_to_all_single(
                remote_edges_pre_post_aggr_to, remote_edges_pre_post_aggr_from, recv_splits, send_splits
            )
        ...
        (
            ...
            begin_edge_on_each_partition_to,
            pre_post_aggr_to_splits,
        ) = DataProcessorForPreAggresive.process_remote_edges_pre_post_aggr_to(
            remote_edges_pre_post_aggr_to, world_size
        )
