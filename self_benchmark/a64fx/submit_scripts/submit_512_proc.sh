...
graph_name="products"
dir_stdout="../log/int8/ogbn_${graph_name}_log/${PJM_MPI_PROC}proc/"
...
# LD_PRELOAD=/home/ra000012/a04083/gnn/gnn/pytorch/scripts/fujitsu/lib/libtcmalloc.so mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}5/stdout -stderr-proc ${dir_stdout}5/stderr python ../dist_pyg_test.py --graph_name=${graph_name} --model=sage --is_async=true --input_dir=${input_dir} --random_seed=${random_seed} --is_fp16=${is_fp16}
...
LD_PRELOAD=/home/ra000012/a04083/gnn/gnn/pytorch/scripts/fujitsu/lib/libtcmalloc.so mpirun -np ${PJM_MPI_PROC} -stdout-proc ${dir_stdout}2/stdout -stderr-proc ${dir_stdout}2/stderr python python ../train.py --config=../config/ogbn-products.yaml --is_fp16=true
