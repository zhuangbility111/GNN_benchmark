...
class QuantizerForPureBits(object):
    ...
    def quantize_fp32_to_intX(data_fp32, data_int8, num_bits=8):
        ...
        computation_ops.quantize_tensor(data_fp32, data_int8, zero_point, scale, num_bits)
        ...
        TimeRecorder.print_time(
            dist.get_rank(),
            "inner prepare data for quantization(ms): ",
            (inner_quantize_begin - total_quantize_begin) * 1000.0,
        )
        TimeRecorder.print_time(
            dist.get_rank(),
        ...
        TimeRecorder.print_time(
            dist.get_rank(), "inner stack data(ms): ", (total_quantize_end - inner_quantize_end) * 1000.0
        )
        TimeRecorder.print_time(
    ...
    def dequantize_intX_to_fp32(data_int8, data_fp32, scale, zero_point, num_bits=8):
        ...
        computation_ops.dequantize_tensor(data_int8, data_fp32, zero_point, scale, num_bits)
