...
# LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD sh run_dist.sh -n $NP -ppn $NPP python ../train.py --config=../config/ogbn-test.yaml --is_pre_delay=true
# LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD sh run_dist.sh -n $NP -ppn $NPP python ../train.py --config=../config/ogbn-arxiv.yaml --is_fp16=true --is_pre_delay=true
LD_PRELOAD=$tcmalloc_path:$LD_PRELOAD sh run_dist.sh -n $NP -ppn $NPP python test_bug.py
