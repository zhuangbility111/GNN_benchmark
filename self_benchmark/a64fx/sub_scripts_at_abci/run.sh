...
# eval $cmd
sh run_dist.sh -n ${num_nodes} -ppn 1 ../python dist_pyg_test.py --graph_name=${graph_name} --model=sage --is_async=false --input_dir=../dataset/ogbn_${graph_name}_new/ogbn_${graph_name}_${num_nodes}_part --random_seed=${random_seed} --is_fp16=${is_fp16} --num_epochs=20
